...
# =====================================================
# LOAD SPACY MODEL — safe loading with fallback
# =====================================================
@st.cache_resource
...
# =====================================================
METAPHOR_WORDS = [
    "battle", "fight", "war", "enemy", "attack",
    "combat", "storm", "wave", "drowning", "tsunami"
]
EVALUATION_WORDS = [
...
def concordance(text, keyword, width=50):
    results = []
    for match in re.finditer(keyword, str(text), re.IGNORECASE):
...
if uploaded_file is not None:
    ...
    # Pre-compute classification outside tabs so Tab 4 & 5 both have access
    ...
    with tab1:
        ...
        col2.metric("Total Words", len(processed_tokens))
        col3.metric("Unique Words", len(set(processed_tokens)))
    ...
    with tab2:
        ...
        fig = px.bar(freq_df, x="Word", y="Frequency", title="Top 20 Frequent Words")
        st.plotly_chart(fig, use_container_width=True)
        st.write("### Word Cloud")
        wc = WordCloud(width=1000, height=500, background_color="white").generate(
    ...
    with tab5:
        ...
        if category_counts:
            if "Metaphor" in category_counts:
                st.info("Flood discourse frequently uses metaphorical framing, representing floods as conflict, warfare, or destructive force.")
            if "Evaluation" in category_counts:
                st.info("Evaluative language indicates emotional and ideological positioning within disaster reporting.")
            if "Identity" in category_counts:
                st.info("Identity constructions reveal how social actors such as victims, authorities, and communities are represented.")
            if "Erasure" in category_counts:
                st.info("Erasure patterns suggest passive constructions that may hide responsibility or agency.")
            if "Ideology" in category_counts:
                st.info("Ideological patterns reflect underlying assumptions about development, governance, climate, and responsibility.")
        else:
